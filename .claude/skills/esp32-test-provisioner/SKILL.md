...
4. **Add the bridge's group address** (the erase wiped it too): `curl -X POST http://<bridge-ip>/api/mesh/settings -d '{"group_addr":"0xC000"}'` — this adds it to the (up to 3-entry) group address list and calls `ble_mesh_subscribe_group_addr()` immediately, no reboot needed. `GET /api/mesh/settings` lists what's configured; `POST /api/mesh/settings/remove` with the same body drops one. Get the bridge's IP from its own boot log (`wifi_provisioning: Got IP address: ...`) or `wifi_status` over serial.
