...
The device is either a mesh Provisioner (standalone mode, self-generates NetKey/AppKey/address) or a mesh Node (join-existing mode, provisioned by an external provisioner like nRF Mesh). **Both roles cannot be compiled in together** — `bt_mesh_init()` unconditionally initializes Provisioner-role internals whenever `CONFIG_BLE_MESH_PROVISIONER` is set, which crashes (`Guru Meditation`) if the runtime role is actually Node. Provisioner-only code is therefore guarded with `#ifdef CONFIG_BLE_MESH_PROVISIONER` throughout (`ble_mesh_control.cpp`, `ble_mesh_provisioning.cpp`, `ble_mesh_node.cpp`, `web_server.cpp`, `mqtt_control.cpp`) rather than deleted, so the standalone SKU keeps working when that config is re-enabled. Which mode is active at runtime is a separate, persisted setting (`mesh_config_t.mode`, `MESH_MODE_STANDALONE` vs `MESH_MODE_JOIN_EXISTING`; unset defaults to whichever role the SKU compiles in, and the setup wizard refuses the other) — the sdkconfig flag controls what's *possible to compile*, the persisted mode controls what actually *runs*. The tracked `sdkconfig.defaults` on `main` selects the Provisioner SKU (what release builds ship); switch it locally to work on the Node SKU.

### Two NVS namespaces, easy to conflate

- `mesh_cfg` (`MESH_CONFIG_NAMESPACE` in `wifi/mesh_config.*`): this app's own config — mode, net/app key (standalone mode only), up to `MESH_MAX_GROUP_ADDRS` (3, matching `CONFIG_BLE_MESH_MODEL_GROUP_COUNT`'s per-model subscription list depth) group addresses, and (join-existing mode) the node identity learned from a real provisioning handshake (`node_addr`, `node_net_idx`, `node_app_idx`). `mesh_config_add_group_addr`/`_remove_group_addr` are the API — a legacy single-address NVS key from before multi-group support is transparently migrated to a 1-item list on load.
...
"External Mesh Nodes" (`ble_mesh_discover_external_nodes` and friends in `ble_mesh_control.cpp`) are devices on a joined mesh that this bridge never provisioned — no DevKey means Composition Data Get is unreachable by spec (Config Server only ever accepts the DevKey). Capability discovery instead probes each model type with a Get to every configured group address and records who answers as the closest available substitute. This is a distinct data structure (`external_mesh_node_t` / `external_nodes` vector) and code path from `node_manager()`, even though both render as similar-looking cards in the dashboard.

External Mesh Nodes also get their own parallel MQTT/HA integration (`mqtt/mqtt_external_control.*`) rather than reuse of `mqtt_control.cpp`'s `bm2mqtt_node_info`-based functions — they're keyed by unicast address (no DevKey/UUID to key on). Feature-for-feature it matches the provisioned-node flow, including HSL/CTL ranges: the first time a node's Lightness/HSL/CTL feature is detected, `ble_mesh_control.cpp` fires a one-shot unicast Range Get for it (`send_external_lightness_range_get` / `_hsl_range_get` / `_ctl_temperature_range_get`) — an AppKey-bound op, not DevKey-bound, so it works even without having provisioned the node. Until that reply arrives, `external_mesh_node_t`'s range fields hold placeholder defaults (full 0–65535 for hue/saturation, 800–20000K for CTL — the Mesh spec's own unconfigured default); once it arrives, discovery gets re-announced (`mqtt_notify_external_node_changed(addr, true)`) so HA picks up the real `brightness_scale`/`min_kelvin`/`max_kelvin`. Topics are `<bridge base>/ext_<addr hex>/{set,state,set_position}`; discovery IDs are `homeassistant/{light,cover}/blemesh2mqtt_ext_<addr hex>_{light,cover}/config`. `upsert_external_node_*` functions call `mqtt_notify_external_node_changed()` on every state-changing probe reply — HA discovery fires on first sighting, a status publish on every change after that.
...
`web_server/web_server.cpp` serves both the dashboard/API (once WiFi is configured) and, via `wifi/wifi_provisioning.cpp`, a captive-portal setup wizard for first-time WiFi configuration. Static assets (`main/littlefs/*`) are packed into a LittleFS image and flashed as a separate `storage` partition (see `littlefs_create_partition_image` in `main/CMakeLists.txt`) — editing `index.html`/`setup.html`/`js/main.js` requires reflashing to take effect, not just a app rebuild (though a normal `flash` does both). The build stages those assets and pre-gzips `css/style.css` and `js/main.js` (served with `Content-Encoding: gzip` by `static_handler`) so the image fits the 224K `storage` partition — keep it that size: a partition-table change can't be delivered over OTA. `POST /api/ota/upload_bundle` takes a combined firmware + web-interface image built by `tools/make_update_bundle.py`.

**Developer-only tooling is compiled in only with `CONFIG_BM2MQTT_DEBUG_TOOLS=y`** (Kconfig, `main/Kconfig.projbuild`, default off so release/shared binaries don't carry it): `GET /api/mesh/debug`, `GET /api/logs` + `log_history`, `POST /api/mesh/reset_role`, and the `wifi_set` console command. The tracked `sdkconfig.defaults` leaves it off — add `CONFIG_BM2MQTT_DEBUG_TOOLS=y` to your local `sdkconfig.defaults` (don't commit it) when you need them. The dashboard's console/`/ws/logs` and `/api/mesh/keys` are user features and stay in all builds.

`GET /api/mesh/debug` (`mesh_debug_status_handler`) dumps the mesh stack's internal state as JSON in one shot — SKU, mode, `local_element_addr`/`net_idx`/`app_idx`, group addresses, whether local keys actually resolve, and the external-nodes table — for diagnosing live without a serial capture.

External Mesh Node endpoints: `GET /api/mesh/external/nodes`, and POST-only `/api/mesh/external/{discover,command,mqtt}` (`mqtt` re-publishes one node's HA discovery + status — the dashboard's "MQTT Discovery" button). `POST /api/mesh/reset_role` (debug builds only) is POST-only too.

`GET/POST /api/mesh/settings` and `POST /api/mesh/settings/remove` manage the group address list — `POST` adds (up to `MESH_MAX_GROUP_ADDRS`, `{"group_addr": "0xC000"}`), `remove` drops one; both subscribe/unsubscribe the bridge's Client models immediately, no restart needed.
