...
External Mesh Node endpoints: `GET /api/mesh/external/nodes`, and POST-only `/api/mesh/external/{discover,command,mqtt}` (`mqtt` re-publishes one node's HA discovery + status — the dashboard's "MQTT Discovery" button). `POST /api/mesh/reset_role` is POST-only too.
