...
External Mesh Nodes also get their own parallel MQTT/HA integration (`mqtt/mqtt_external_control.*`) rather than reuse of `mqtt_control.cpp`'s `bm2mqtt_node_info`-based functions — they're keyed by unicast address (no DevKey/UUID to key on). Feature-for-feature it matches the provisioned-node flow: onoff, lightness-as-brightness, generic-level-as-cover, HSL (hs color), and CTL (color temp) all work via `ble_mesh_send_external_hsl_command`/`ble_mesh_send_external_ctl_command`. The one gap is per-device HSL/CTL *ranges*: provisioned nodes get theirs from a Range Get (an AppKey-bound, not DevKey-bound, unicast op — so it'd work for external nodes too, it's just not wired up), so external nodes fall back to hardcoded defaults (hue/saturation as full 0–65535 mapped to HA's 0–360/0–100, CTL kelvin range as the Mesh spec's own unconfigured default of 800–20000K) instead of the device's real range. Topics are `<bridge base>/ext_<addr hex>/{set,state,set_position}`; discovery IDs are `homeassistant/{light,cover}/blemesh2mqtt_ext_<addr hex>_{light,cover}/config`. `ble_mesh_control.cpp`'s `upsert_external_node_*` functions call `mqtt_notify_external_node_changed()` on every state-changing probe reply — HA discovery fires on first sighting, a status publish on every change after that.
