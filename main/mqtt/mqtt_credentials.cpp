...
std::string MqttCredentialManager::get_connection_state_string() const
{
    ...
    {
    case mqtt_connection_state_t::UNCONFIGURED:
        return "unconfigured";
    case mqtt_connection_state_t::CONFIGURED:
        return "configured";
    case mqtt_connection_state_t::CONNECTING:
        return "connecting";
    case mqtt_connection_state_t::CONNECTED:
        return "connected";
    case mqtt_connection_state_t::DISCONNECTED:
        return "disconnected";
    case mqtt_connection_state_t::ERROR_AUTH:
        return "auth_error";
    case mqtt_connection_state_t::ERROR_NETWORK:
        return "network_error";
    case mqtt_connection_state_t::ERROR_TIMEOUT:
...
static bool is_valid_hostname(const std::string &hostname)
{
    ...
    for (size_t i = 0; i < hostname.length(); i++)
    {
        ...
        if (!std::isalnum(c) && c != '.' && c != '-')
            return false;
        if (c == '-' && (i == 0 || hostname[i - 1] == '.' || (i + 1 < hostname.length() && hostname[i + 1] == '.')))
            return false;
    }
