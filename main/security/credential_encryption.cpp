...
credential_encryption &credential_encryption::instance()
{
    static credential_encryption instance;
    return instance;
}
...
esp_err_t credential_encryption::initialize()
{
    if (initialized_)
    {
        return ESP_OK;
...
esp_err_t credential_encryption::setup_encryption_key()
{
...
void credential_encryption::generate_iv(uint8_t *iv, size_t length)
{
    esp_fill_random(iv, length);
}

esp_err_t credential_encryption::encrypt_string(const std::string &plaintext, std::string &ciphertext)
{
    if (!initialized_)
    {
...
esp_err_t credential_encryption::decrypt_string(const std::string &ciphertext, std::string &plaintext)
{
    if (!initialized_)
    {
        LOG_ERROR(TAG, "Encryption not initialized");
    ...
    if (decoded_len < AES_BLOCK_SIZE + AES_BLOCK_SIZE)
    { // IV + at least one block
